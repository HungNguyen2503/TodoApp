...
export default function App() {
  ...
  useEffect(() => {
    ...
    document.title =
      remaining > 0 ? `Todo List (${remaining} pending)` : "Todo List";
  }, [todos]);
  ...
  const toggleTodo = async (_id) => {
    setTodos((prev) =>
      prev.map((t) => (t._id === _id ? { ...t, done: !t.done } : t))
    );
    try {
  ...
  return (
    <div className="app-shell">
      ...
      >
        <header className="app-header">
          <div className="title-wrap">
            <motion.h1 layoutId="title" className="gradient-text">
              Todo List
            </motion.h1>
            <span className="count-chip" aria-label="Total tasks">
              {todos.length}
            </span>
          </div>
          <div className="header-actions">
            {token && <span className="user-chip">Hi! {username}</span>}
            <button
      ...
      <footer className="app-footer">NguyenPhiHung</footer>
    </div>
  );
}
...
function AuthPanel({ onSubmit, loading, error }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    onSubmit(mode === "login" ? "login" : "signup", form);
  };
  return (
    <motion.div
